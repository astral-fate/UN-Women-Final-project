        ...
        <div class="mx-2">
            ...
            <form action="{{ route('admin.testimonials.update', $testimonial->id) }}" method="POST" enctype="multipart/form-data" class="px-md-5">
                ...
                <div class="form-group mb-3 row">
                    ...
                    <div class="col-md-10">
                        <input type="text" name="name" id="name" value="{{ old('name', $testimonial->name) }}" placeholder="e.g. John Doe" class="form-control py-2 @error('name') is-invalid @enderror" required />
                        @error('name')
                ...
                <div class="form-group mb-3 row">
                    ...
                    <div class="col-md-10">
                        <textarea name="content" id="content" rows="5" class="form-control @error('content') is-invalid @enderror" required>{{ old('content', $testimonial->content) }}</textarea>
                        @error('content')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="form-group mb-3 row">
                    ...
                    <div class="col-md-10">
                        <input type="checkbox" name="is_published" id="is_published" value="1" class="form-check-input" style="padding: 0.7rem;" {{ old('is_published', $testimonial->is_published) ? 'checked' : '' }} />
                    </div>
                </div>
                <hr>
                <div class="form-group mb-3 row">
                    ...
                    <div class="col-md-10">
                        <input type="file" name="image" id="image" class="form-control @error('image') is-invalid @enderror" style="padding: 0.7rem; margin-bottom: 10px;" />
                        @error('image')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        @if($testimonial->image)
                            <img src="{{ asset('storage/' . $testimonial->image) }}" alt="Current Testimonial Image" style="width: 10rem;">
                        @else
